...
# Start with network access (for team sharing)
npm run start:host
```
...
npm run start:host
```
Access at: `http://localhost:4200` or `http://[your-ip]:4200`
...
To share with team members on same network:
...
# Run container
docker run -p 80:80 kms-website

# Run in background
docker run -d -p 80:80 --name kms-site kms-website
```
...
- [ ] [Set up project integrations](https://rsource.instantmseloans.in/nikul.patel/kms-website/-/settings/integrations)
